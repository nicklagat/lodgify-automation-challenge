class UpdateWebPage {
  constructor() {
    ...
    this.submitButton =
      '[type="submit"][aria-disabled="false"][class^="_8313bd46"]';
  ...
  addProject() {
    cy.get(this.addButton)
      .click()
      .then(() => {
        cy.get(this.modalOverlay)
          .find(this.projectNameField)
  ...
  selectProject() {
    cy.get(this.projectsList, { timeout: 90000 }).then((listItems) => {
      const validItem = [...listItems].find((item) =>
        item.textContent.includes("Software Engineering Project")
      );
      if (validItem) {
        cy.wrap(validItem).click();
      } else {
        throw new Error(
          "No project found with the name Software Engineering Project"
        );
      }
    });
  }
  ...
  updateFirstTask(description) {
    cy.get(this.itemList)
      .first()
      .click()
      .then(() => {
        cy.get(this.modalOverlay)
          .find('[data-testid="task-details-modal"]')
          .click();
